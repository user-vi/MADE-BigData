...
df = spark.sql(
        """
        select from_unixtime(cast(ts as bigint),'yyyy-MM-dd HH:mm:ss.SSS') as ts,
        case when substr(url, -3) like '.ru' then 'ru' else 'not ru' end as zone,
        uid
        from logs
        """
)

df.createOrReplaceTempView("logs_2")
res = spark.sql(
        """
        select
            window(ts, "2 seconds", "1 seconds") as window,
            zone,
            count(uid) as view,
            approx_count_distinct(uid) as unique
        from logs_2
        group by window, zone
        order by window, view desc 
        limit 20
        """
)
